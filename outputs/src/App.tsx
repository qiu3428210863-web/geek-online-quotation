...
const sectionCopy: Record<(typeof sections)[number][0], { eyebrow: string; title: string; body: string }> = {
  company: { eyebrow: 'CHAPTER 01', title: '把品牌愿景，变成可执行的体验。', body: '从策略、视觉到数字产品，我们为客户公司搭建一套清晰而有生命力的品牌系统。' },
  cases: { eyebrow: 'CHAPTER 02', title: '每一次交付，都是一次长期合作的起点。', body: '精选品牌与数字体验案例，展示从问题洞察到落地上线的完整路径。' },
  features: { eyebrow: 'CHAPTER 03', title: '清晰的功能清单，让范围一目了然。', body: '将目标拆解为可验证的功能模块，减少沟通成本，让项目始终朝着同一个方向前进。' },
  pricing: { eyebrow: 'CHAPTER 04', title: '报价透明，决策更笃定。', body: '每一项投入都对应具体产出，按阶段拆分计划与预算，便于团队灵活安排。' },
  meeting: { eyebrow: 'CHAPTER 05', title: '在一次次对话里，找到正确答案。', body: '通过结构化会议同步目标、节奏与反馈，让每个关键节点都被充分理解。' },
  process: { eyebrow: 'CHAPTER 06', title: '从第一张草图到最终上线。', body: '策略、设计、开发、测试与交付环环相扣，建立可持续迭代的工作流。' },
  'after-sales': { eyebrow: 'CHAPTER 07', title: '上线之后，我们仍然在场。', body: '持续维护、数据观察与体验优化，让产品稳定运行并持续增长。' },
}
        ...
        {sections.slice(2).map(([id]) => {
          ...
          if (id === 'meeting') return <motion.section className="content-section meeting-content" id={id} key={id} initial={{ opacity: 0, y: 34 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: .14 }} transition={{ duration: .72, ease: [.22, 1, .36, 1] }}>
            <div className="meeting-inner">
              <div className="meeting-section-heading">
                <p className="section-eyebrow">CHAPTER 05</p>
                <h2>关于会议</h2>
              </div>
              ...
              <div className="process-layout">
                <div className="process-copy">
                  <div className="process-heading">
                    <p className="section-eyebrow">CHAPTER 06</p>
                    <h2>开发流程说明</h2>
          ...
          if (id === 'pricing') return <motion.section className="content-section pricing-content" id={id} key={id} initial={{ opacity: 0, y: 34 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: .14 }} transition={{ duration: .72, ease: [.22, 1, .36, 1] }}>
            ...
            <div className="pricing-title-wrap">
              <p className="section-eyebrow">CHAPTER 04</p>
              <h2>定制开发报价清单</h2>
              ...
              <section className="support-library" id="support" aria-labelledby="support-library-title">
                ...
                <div className="support-notice">
                  <p className="support-notice-lead">自上线之日起，<strong>首年赠送一年技术运维服务</strong>，确保应用程序可正常使用。</p>
                  <p className="support-notice-highlight"><strong>服务包含技术维保和非技术维保</strong>，将由客户成功团队全程负责。</p>
                  <p className="support-notice-detail">对于故障响应时间视问题的紧急程度进行处理。</p>
                </div>
        ...
        <motion.section className="after-sales-section" id="after-sales" initial={{ opacity: 0, y: 34 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: .14 }} transition={{ duration: .72, ease: [.22, 1, .36, 1] }}>
          <div className="after-sales-inner">
            <div className="after-sales-heading">
              <p className="section-eyebrow">CHAPTER 07</p>
              <h2>售后服务说明</h2>
            </div>
                ...
                <AnimatePresence mode="wait" initial={false}>
                  <motion.div key={afterSalesCategory} className="after-sales-category-content" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: .26, ease: [.22, 1, .36, 1] }}>
                    ...
                    {afterSalesCategory === 'deliverables' && <>
                      ...
                      <div className="after-sales-info-cards deliverables-cards">{deliveryRows.map(([title, detail], index) => <article className="after-sales-info-card" key={title}><div className="after-sales-info-card-top"><span>0{index + 1}</span><strong>{title}</strong></div><div className="deliverable-list" aria-label={`${title}交付物`}>{detail.split('；').map((item) => item.trim()).filter(Boolean).map((item) => <span className="deliverable-item" key={item}><i aria-hidden="true">•</i>{item}</span>)}</div></article>)}</div>
                    </>}
                    {afterSalesCategory === 'testing' && <>
                      <h3>测试说明</h3>
                      <div className="after-sales-testing-overview">
                        <div className="after-sales-testing-overview-label"><span>测试流程说明</span><b>QUALITY GATE</b></div>
                        <div className="after-sales-testing-overview-copy">
                          <h4>严谨的项目测试说明</h4>
                          <p>为确保产品上线质量，极客上线建立系统化测试流程，覆盖功能、性能、安全、兼容等关键维度，并以完整测试报告和验收记录保障交付可控、上线可验。</p>
                        </div>
                      </div>
                      <div className="after-sales-testing-cards" aria-label="完整测试流程">{testingRows.map(([stage, work, owner, prerequisite, output], index) => <article className="after-sales-testing-card" key={`${stage}-${work}`}><span className="after-sales-testing-step">0{index + 1}</span><div className="after-sales-testing-card-top"><strong>{stage}</strong><span>{owner}</span></div><p>{work}</p><div className="after-sales-testing-meta"><span><b>前置条件</b>{prerequisite}</span><span><b>产出物</b>{output}</span></div></article>)}</div>
                    </>}
